...
#Set paths to reference fasta, sample list and file specifying paths to dirctories containing demultiplexed reads
...
#Calculate length of genome by summing the chromosome / scaffold lengths in ".fai file"
...
if [ -e $R1 ] && [ -e $R2 ]; then
    ...
    samtools depth ${SAMPLE_ID}.sort.bam | awk -v len=${REFGENOME_LENGTH} '{sum+=$3} END { print sum/len}' > ${SAMPLE_ID}.depth
fi
